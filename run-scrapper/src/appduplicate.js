...
async function getCategoriesWithQuery(scrapper) {
    ...
    // console.log(JSON.parse(categoryURLs._remoteObject.value), '**--*--');
...
async function preActionCalled(scrapper, page, categoryPageindex, preActionIndex) {
    ...
    if (preAction.eventName === 'click') {
        ...
        await loadMoreProducts(scrapper, page);
...
async function loadMoreProducts(scrapper, page) {
...
async function getProductsLink(scrapper, productUrl) {
    ...
    for (var i = 0; i < productUrl.length; i = i + 32) {
        // if (i + 10 <= productUrl.length) {
        await get10ProductsAtOnce(scrapper, productUrl.slice(i, i + 32));
        // }
        // else {
